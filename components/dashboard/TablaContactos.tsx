...
const estadoStyles: Record<Contacto['estado'], { bg: string; text: string }> = {
  pendiente:  { bg: 'bg-slate-800',    text: 'text-slate-400' },
  enviado:    { bg: 'bg-blue-950',     text: 'text-blue-400' },
  abierto:    { bg: 'bg-amber-950',    text: 'text-amber-400' },
  respondido: { bg: 'bg-emerald-950',  text: 'text-emerald-400' },
  rebotado:   { bg: 'bg-red-950',      text: 'text-red-400' },
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-400 hover:text-slate-200 bg-white/5 hover:bg-white/8 border border-white/5 rounded-md transition-colors duration-150"
        >
          ...
          <tbody>
            ...
            {contactos.map((c) => {
              ...
              return (
                ...
                >
                  <td className="px-5 py-3.5 font-medium text-white">{c.empresa || '—'}</td>
                  <td className="px-5 py-3.5 text-slate-200">
                    {[c.nombre, c.apellido].filter(Boolean).join(' ') || '—'}
                  </td>
                  <td className="px-5 py-3.5 text-slate-400">{c.cargo || '—'}</td>
                    ...
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-100">
                      ...
                      {onExcluir && (
                        <button
                          ...
                          className="px-2.5 py-1 text-xs text-red-500 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors duration-100"
                        >
                          Excluir
                        </button>
                      )}
